...
`.bin` 回传只针对配置的一个 `.bin` 文件。启动同步时，软件会把 VM 当前已有的 `.bin` 作为基线，不会立即拉回来覆盖宿主机旧文件；启动后 `.bin` 内容变化时，才会回传并覆盖宿主机固件回传目录里的同名文件。如果只是时间变化但内容没变，不会覆盖，并会像“固件就绪”一样通过托盘通知提示。停止同步后，迟到的 `.bin` 轮询结果不会再触发日志、通知或覆盖。
