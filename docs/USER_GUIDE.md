...
增量同步只处理宿主机里新增或修改的文件，并且只同步配置中 `watch_extensions` 包含的扩展名。同步时会先把文件复制到 VM 目标目录下的临时文件，再移动覆盖最终路径，以降低暂停或退出时直接写坏目标文件的风险。同相对路径的 VM 文件会被覆盖；删除、重命名和不在扩展名列表里的文件不会自动同步。
