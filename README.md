...
- 支持同一个 VM、同一个 VM 账号下同时监听两个独立 Keil 工程；项目 1 和项目 2 各自保存 Host 工程路径、VM 工程路径、`.bin` 相对路径和固件回传目录。
...
- 增量同步会先写入 VM 目标目录下的临时文件，再移动覆盖最终文件，降低中断时半写目标文件的风险。
- 监听配置的 VM `.bin` 输出文件，仅在内容变化时回传到宿主机。
- 启动同步时记录 VM 内已有 `.bin` 作为基线，避免旧固件立即覆盖宿主机文件。
- 两个项目的 watcher、上传队列、hash 基线、`.bin` 基线、回传目录和日志栏互相独立；`vmrun` 调用串行执行以降低 VMware VIX 不稳定风险。
